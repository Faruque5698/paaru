      ...
      <div class="col">
        <table id="datatable" class="table table-bordered table-hover table-responsive-sm text-center">
          ...
          <tbody>
            ...
            @if($activitiy_types->count() > 0)
            @foreach($activitiy_types as $key =>$type)
            ...
            <tr>
              ...
              <td>
                ...
                <a href="{{ route("backend.$route_path.edit", $type->id) }}" class="btn btn-success btn-sm" title="Edit">
              ...
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <form action="{{ route("backend.$route_path.unpublished", $type->id) }}" method="POST">
                      {{ csrf_field() }}
                      {{ method_field('PUT') }}
            ...
            <div class="modal fade" id="publishModal{{$type->id}}">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <form action="{{ route("backend.$route_path.published", $type->id) }}" method="POST">
                      {{ csrf_field() }}
                      {{ method_field('PUT') }}
                      <button type="submit" class="btn btn-danger">Yes</button>
                      <button type="button" class="btn btn-primary" data-dismiss="modal">No</button>
            ...
            <div id="deleteModal{{$type->id}}" class="modal fade">
              <div class="modal-dialog modal-confirm">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <form action="{{ route("backend.$route_path.destroy", $type->id) }}" method="POST">
                      {{ csrf_field() }}
                      {{ method_field('DELETE') }}
