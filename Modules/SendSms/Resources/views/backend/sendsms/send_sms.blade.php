  ...
  <div class="card-body">
    ...
    <div class="row mt-4">
      <div class="col">
        ...
        {{ html()->form('POST', route("backend.$module_name.sendMessageToMembers"))->class('form')->class('form-horizontal')->open() }}
        ...
        <div class="row">
          ...
          <div class="col-6">
            <div class="form-group">
              ...
              <select name="member" class="form-control select2">
                ...
                <option value="2">
                  {{ 'Alumni Member' }} {{ - ( $alumni_members) }}
               </option>
               
               <option value="3">
                  {{ 'Life Member' }} {{ - ( $life_time_members) }}
                </option>

                <option value="4">
                {{ 'Execuitive Committee' }} {{ - ( $count_exective_committee ) }}
              </option>
          ...
          <div class="col-6">
            <div class="form-group">
              ...
              {{ html()->label($field_lable, $field_name) }} {!! fielf_required($required) !!}
              {{ html()->text($field_name)->value($sendsms->$field_name)->placeholder($field_placeholder)->class('form-control')->attributes(["$required"]) }}
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-6">
            <div class="form-group">
              {{ html()->button($text = "<i class='fas fa-comment'></i> " . ucfirst($module_action) . "", $type = 'submit')->class('btn btn-success') }}
            </div>
          </div>
          <div class="col-6">
...
var sms_message = {!! json_encode($sendsms->toArray()) !!};

$(window).on('load', function(){

  var sms_count = 0;

  document.getElementById("message").value = sms_message.message;

  if(sms_message.message.length > 159){
    sms_count = sms_message.message.length / 159;
    document.getElementById("text_count").innerHTML = "( "+Math.ceil(sms_count)+" SMS)" ;
  }else{
    document.getElementById("text_count").innerHTML = "( "+sms_message.message.length+" )" ;
  }
