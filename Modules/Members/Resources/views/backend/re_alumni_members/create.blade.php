...
<script type="text/javascript">
  ...
  $(document).on('click', '#submit', function(e){
    ...
    var re_al_member_content = CKEDITOR.instances['re_al_member_content'].getData();
    var re_al_member_msg = CKEDITOR.instances['re_al_member_msg'].getData();
    //To receive data from without ckeditor...
    var video_link = document.getElementById("video_link").value;

    $.ajax({
      url: "{{ route('backend.renowned_alumni_members.store') }}",
      method: 'POST',
      data: {
        member_id:member_id,
      ...
      success: function(data) {
        ...
        if(!$.trim(data)){
          alert("Failed to create Renowned Alumni Members!!!");
        }else{
