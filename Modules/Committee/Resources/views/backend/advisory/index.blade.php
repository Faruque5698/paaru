        ...
        <table id="datatable" class="table table-bordered table-hover table-responsive-sm text-center">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $a_committee->member->occupation }}</td>
              <td>
                <img src="{{ asset('assets/images/members/' .$a_committee->member->image) }}" class="img-fluid img-thumbnail" style="max-width:100px;" alt="{{ $a_committee->member->name }}">
              </td>
              <td>
                {{ $a_committee->users_created_by->name }}
              </td>
              <td>
                <a class="btn btn-danger btn-sm" data-toggle="modal" data-target="#deleteModal{{$a_committee->id}}" title="Delete">
                  <span class="fas fa-trash"></span>
                </a>

                <a href="{{ route("backend.$route_path.edit", $a_committee->member->id) }}" class="btn btn-success btn-sm" title="Edit">
            ...
            <!-- Delete Modal -->
            <div id="deleteModal{{$a_committee->id}}" class="modal fade">
              <div class="modal-dialog modal-confirm">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <form action="{{ route("backend.$route_path.destroy", $a_committee->id) }}" method="POST">
                      {{ csrf_field() }}
                      {{ method_field('DELETE') }}
